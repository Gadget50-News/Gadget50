...
$related = $pdo->query("SELECT n.id, n.title, n.image, n.created_at, c.name AS category_name FROM news n LEFT JOIN categories c ON c.id = n.category_id WHERE n.status = 'published' AND n.id != " . (int) $item['id'] . " ORDER BY n.created_at DESC LIMIT 4")->fetchAll();
$mostRead = $pdo->query("SELECT n.id, n.title, c.name AS category_name FROM news n LEFT JOIN categories c ON c.id = n.category_id WHERE n.status = 'published' ORDER BY n.views DESC, n.created_at DESC LIMIT 4")->fetchAll();
?>
<!doctype html>
<html lang="en">
<head>
                    ...
                    <?php foreach ($related as $story): ?>
                        <div class="col-md-6">
                            <article class="mini-story">
                                <a href="news.php?id=<?= (int) $story['id'] ?>">
                                ...
                                <h4><a href="news.php?id=<?= (int) $story['id'] ?>"><?= e((string) $story['title']) ?></a></h4>
                            </article>
                        </div>
                    <?php endforeach; ?>
                </div>
