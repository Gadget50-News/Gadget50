...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($name === '' || strlen($name) > 120) {
            setFlash('danger', 'Enter a category name of 120 characters or fewer.');
            redirect('categories.php');
        }
        ...
        try {
            ...
            setFlash('danger', 'That category slug already exists.');
        }
    }
...
$categories = $pdo->query('SELECT id, name, slug, description FROM categories ORDER BY name ASC')->fetchAll();
$flash = getFlash();
?>
<!doctype html>
<html lang="en">
    ...
    <main class="container py-5">
        ...
        <div class="card border-0 shadow-sm mb-4"><div class="card-body">
            <form method="post" class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label">Slug</label><input class="form-control" name="slug" maxlength="120"></div>
                <div class="col-md-4"><label class="form-label">Description</label><input class="form-control" name="description"></div>
                <div class="col-12"><button class="btn btn-primary">Add Category</button></div>
            </form>
        </div></div>
        <div class="card border-0 shadow-sm"><div class="card-body"><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Name</th><th>Slug</th><th>Action</th></tr></thead><tbody>
        <?php foreach ($categories as $category): ?><tr><td><?= e((string) $category['name']) ?></td><td><?= e((string) $category['slug']) ?></td><td><form method="post" onsubmit="return confirm('Delete this category?');"><input type="hidden" name="csrf_token" value="<?= e(csrfToken()) ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int) $category['id'] ?>"><button class="btn btn-sm btn-danger">Delete</button></form></td></tr><?php endforeach; ?>
